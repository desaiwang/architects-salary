class ClickableHistogramSlider {
  ...
  initialize() {
    ...
    if (Array.isArray(this.options.sortOrder)) {
      ...
      this.groupCounts.sort((a, b) =>
        this.options.ascending ? a.key - b.key : b.key - a.key
      );
    } else if (this.options.sortOrder === "count") {
      this.groupCounts.sort((a, b) =>
        this.options.ascending ? a.count - b.count : b.count - a.count
      );
    }
    ...
    if (this.options.colorInterpolationType) {
      if (this.options.colorInterpolationType === "sequential discrete") {
        this.colorScale = (x) =>
          this.options.colorInterpolator(
            d3.scalePoint().domain(this.uniqueKeys).range([0, 1])(x)
          );
      } else if (
        this.options.colorInterpolationType === "sequential continuous"
      ) {
        this.colorScale = (x) =>
          this.options.colorInterpolator(
            d3.scaleLinear().domain(d3.extent(this.uniqueKeys)).range([0, 1])(x)
          );
      } else if (this.options.colorInterpolationType === "categorical") {
        this.colorScale = (x) =>
          d3
            .scaleOrdinal()
            .domain(this.uniqueKeys)
            .range(this.options.colorScheme)(x);
      }
    }
    ...
    this.groupCounts.forEach(
      (d) =>
        (d.color = this.colorScale
          ? this.colorScale(d.key)
          : ClickableHistogramSlider.greyFill)
    );
  ...
  setupScales() {
    ...
    this.yScale = d3
      .scaleLinear()
      .domain([0, d3.max(this.groupCounts, (d) => d.count)])
      .nice()
      .range([this.sliderHeight - 4, this.options.yBetweenLabelAndHist]);
  }
  ...
  setupSvg() {
    ...
    this.svg = wrapper
      ...
      .attr(
        "height",
        this.sliderHeight + (this.options.rotateAxisLabels ? 50 : 30)
      );
  ...
  colorRect = (d) =>
    d.clicked
      ? this.showColors
        ? d.color
        : ClickableHistogramSlider.greyFill
      : ClickableHistogramSlider.whiteFill;

  colorBorder = (d) =>
    d.clicked
      ? this.showColors
        ? d3.lab(d.color).darker()
        : d3.lab(ClickableHistogramSlider.greyFill).darker()
      : ClickableHistogramSlider.greyedoutColor;
  ...
  setupBrush() {
    ...
    const brushMoved = (event) => {
      if (event.selection !== null) {
        ...
        this.groupCounts.forEach(
          (item) => (item.clicked = this.valueList.includes(item.key))
        );
  ...
  setupLegend(legendWidth, legendHeight, fontSize = this.options.fontsize) {
    ...
    let xScaleLegend = d3
      ...
      .range([1, legendWidth])
      .padding(0.12);
    let gXAxis = legendSvg
      .append("g")
      .attr(
        "transform",
        `translate(${this.options.rotateAxisLabels ? 4 : 0},${
          this.options.rotateAxisLabels ? legendHeight + 2 : legendHeight + 4
        })`
      )
      .call(
        d3
    ...
    if (this.options.rotateAxisLabels) {
      ...
      gXAxis.selectAll("text").style("font-size", fontSize);
    }
  ...
  setupHistogram() {
    let gXAxis = this.svg
      .append("g")
      .attr("transform", `translate(0,${this.sliderHeight + 4})`)
      .call(
        d3
    ...
    if (this.options.rotateAxisLabels) {
      ...
      gXAxis.selectAll("text").style("font-size", this.options.fontsize);
    }
    ...
    this.histRects = this.svg
      ...
      .attr("height", (d) =>
        d.count == 0 ? 0 : this.yScale(0) - this.yScale(d.count) + 5
      )
      .attr("fill", (d) => this.colorRect(d))
      ...
      .on("click", (event, d) => {
        if (event.shiftKey) {
          //alow multi select, and to click to unselect while shift is held
          d.clicked = !d.clicked;
          if (d.clicked) {
          ...
          //only select the clicked bar (like a radio button)
  ...
  update() {
    ...
    const filteredData = this.dataAll.filter((d) =>
      ClickableHistogramSlider.pointPassesFilters(
        this.filters,
        this.attribute,
        d
      )
    ...
    const filteredGroupedData = d3.groups(
      filteredData,
      (d) => d[this.attribute]
    );
    ...
    const countsMap = new Map(
      filteredGroupedData.map(([key, values]) => [key, values.length])
    );
